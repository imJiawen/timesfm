...
class TimesFm(nn.Module):
    ...
    def forward(self, input_ts, freq=None):
        """
        ...
        if freq is None:
            f_emb = self.freq_emb(torch.zeros((bs, 1), dtype=torch.long)).to('cuda')
        else:
            assert freq in [0,1,2], "freq: 0 for high frequency (default), 1 for medium, and 2 for low"
            freq = torch.tensor([freq] * bs).unsqueeze(-1).long()
            f_emb = self.freq_emb(freq).to('cuda')
